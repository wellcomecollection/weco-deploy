...
def warn_if_not_latest_version():
    with urllib.request.urlopen(f"https://pypi.org/pypi/weco-deploy/json") as response:
        try:
            ...
            if current_version < latest_version:
                click.echo(click.style(f"You are using weco-deploy version {current_version_str}. "
                                       f"However, version {latest_version.vstring} is available.", fg="red"))
            ...
            click.echo(click.style(f"Error when checking for latest version:", fg="red"))
            click.echo(e)
            pass
